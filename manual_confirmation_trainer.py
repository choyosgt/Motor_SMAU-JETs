# manual_confirmation_trainer.py - TRAINER CON CONFIRMACIÓN MANUAL OBLIGATORIA
# ACTUALIZADO: Nuevos campos estándar, confirmación manual para todas las detecciones
# MEJORADO: Añade sinónimos y patrones regex precisos automáticamente
...
class ManualConfirmationTrainingSession:
    """Sesión de entrenamiento con confirmación manual obligatoria para todas las detecciones"""
    ...
    def __init__(self, csv_file: str, erp_hint: str = None):
        ...
        # NUEVOS CAMPOS ESTÁNDAR según tu lista
    ...
    def initialize(self) -> bool:
        """Inicializa la sesión de entrenamiento"""
        try:
            print(f"🚀 Initializing MANUAL CONFIRMATION Training Session...")
            print(f"📂 File: {self.csv_file}")
            print(f"🏢 ERP Hint: {self.erp_hint or 'Auto-detect'}")
            print(f"⚠️ ALL detections require manual confirmation")
            print(f"🔧 New standard fields: {len(self.standard_fields)} fields")
    ...
    def run_interactive_training(self) -> Dict:
        """Ejecuta el entrenamiento interactivo con confirmación manual OBLIGATORIA"""
        try:
            print(f"\n🎓 Starting MANUAL CONFIRMATION Training...")
            print(f"=" * 55)
            print(f"⚠️ IMPORTANT: ALL detections will require your confirmation")
            print(f"🧠 System will suggest but YOU decide for every column")
            
            # Análisis inicial del DataFrame
            initial_analysis = self._analyze_initial_state()
            
            # Procesar cada columna CON CONFIRMACIÓN OBLIGATORIA
            for i, column_name in enumerate(self.df.columns, 1):
                print(f"\n📋 Column {i}/{len(self.df.columns)}: '{column_name}'")
                print("-" * 40)
                
                # Obtener muestra de datos
                sample_data = self.df[column_name].dropna().head(20)
                
                # Intentar mapeo automático (solo para sugerencia)
                mapping_result = self._try_automatic_mapping(column_name, sample_data)
                
                if mapping_result:
                    field_type, confidence = mapping_result
                    
                    # SIEMPRE requerir confirmación manual
                    print(f"🤖 SYSTEM SUGGESTION: {column_name} -> {field_type} ({confidence:.3f})")
                    print(f"⚠️ Manual confirmation required for ALL detections")
                    
                    # Comportamiento diferenciado por confianza
                    if confidence > 0.8:
                        print(f"✨ High confidence (> 0.8) - No automatic alternatives suggested")
                        ...
                        print(f"🔍 Low confidence (≤ 0.8) - Searching for intelligent alternatives...")
                        ...
                        # Obtener decisión del usuario con/sin alternativas
                        if alternatives:
                    ...
                    if final_decision:
                        ...
                        # Aprender nuevo patrón o sinónimo si es corrección
                    ...
                    # No se pudo mapear automáticamente
                    print(f"❓ NO AUTOMATIC MAPPING for '{column_name}'")
    ...
    def _try_automatic_mapping(self, column_name: str, sample_data: pd.Series) -> Optional[Tuple[str, float]]:
        """Intenta mapeo automático (solo para sugerencia)"""
        try:
            ...
            if mapping_result:
                ...
                # Verificar que el field_type esté en la lista de campos estándar
                if field_type not in self.standard_fields:
                    print(f"   ⚠️ Field {field_type} not in standard fields, skipping")
                    return None
                ...
                if pattern_boost > 0:
                    ...
                    print(f"   🧠 Pattern boost applied: +{pattern_boost:.2f}")
    ...
    def _find_intelligent_alternatives(self, column_name: str, sample_data: pd.Series, 
        ...
        """Busca alternativas basadas en análisis de contenido"""
        alternatives = []
        
        try:
            # Análisis preliminar de tipo de datos
    ...
    def _get_relevant_standard_field_types(self, data_type_analysis: Dict[str, Any], current_field: str) -> List[str]:
        """Obtiene tipos de campo estándar relevantes basados en el análisis de datos"""
        ...
        # Filtrar el campo actual y solo incluir campos estándar
    ...
    def _analyze_data_type(self, sample_data: pd.Series) -> Dict[str, Any]:
        """Análisis preliminar del tipo de datos"""
        analysis = {
            'is_numeric': False,
            'is_date': False,
            'is_text': False,
        ...
        if not analysis['is_numeric'] or not analysis['is_monetary']:
            try:
                ...
                date_patterns = [
                    ...
                    r'^\d{2}/\d{2}/\d{4}',          # 01/15/2024 (ya incluido arriba, pero común)
                    ...
                    # Formatos con años de 2 dígitos
                    r'^\d{2}/\d{2}/\d{2}',          # 15/01/24
                    r'^\d{1,2}/\d{1,2}/\d{2}',      # 5/1/24 o 15/01/24
                    r'^\d{2}-\d{2}-\d{2}',          # 15-01-24
                    r'^\d{1,2}-\d{1,2}-\d{2}',      # 5-1-24 o 15-01-24
                    
                    # Formatos con nombres de meses (inglés)
                    ...
                    # Formatos con nombres de meses (español)
    ...
    def _analyze_content_for_field_type_improved(self, field_type: str, sample_data: pd.Series, 
        ...
        """Análisis de contenido mejorado usando el análisis de tipo de datos"""
        
        try:
            # Verificar compatibilidad básica con campos estándar
    ...
    def _analyze_amount_content_improved(self, field_type: str, sample_data: pd.Series) -> float:
        """Análisis mejorado para campos de importe"""
        try:
            numeric_data = pd.to_numeric(sample_data, errors='coerce')
            non_null_numeric = numeric_data.dropna()
    ...
    def _analyze_description_content_improved(self, field_type: str, sample_data: pd.Series, 
                                            data_type_analysis: Dict[str, Any]) -> float:
        """Análisis de descripciones mejorado"""
        try:
            str_data = sample_data.astype(str)
            avg_length = str_data.str.len().mean()
    ...
    def _analyze_fiscal_year_content_improved(self, sample_data: pd.Series) -> float:
        """Análisis de año fiscal mejorado"""
        try:
            str_data = sample_data.astype(str)
    ...
    def _get_intelligent_justification(self, field_type: str, sample_data: pd.Series, 
        ...
        """Genera justificación inteligente para una alternativa"""
        try:
            ...
            # Añadir información específica del análisis
            if data_type_analysis['is_monetary']:
    ...
    def _get_high_confidence_user_decision(self, column_name: str, suggested_field: str, 
        ...
        """Obtiene decisión OBLIGATORIA del usuario para alta confianza (sin alternativas automáticas)"""
        
        sample_str = ', '.join([str(x) for x in sample_data.head(5).tolist()])
        print(f"   📊 Sample data: {sample_str}")
        print(f"   🎯 System suggests: {suggested_field} (HIGH confidence: {confidence:.3f})")
        print(f"   ✨ No automatic alternatives shown due to high confidence")
        
        print(f"\n   Choose an option:")
        print(f"   1. ✅ Accept high-confidence suggestion: {suggested_field}")
        print(f"   2. 🎯 Choose different field manually")
        print(f"   3. ⏭️ Skip this column")
        
        while True:
            try:
                user_input = input(f"   👤 Your choice (1-3): ").strip()
                ...
                if choice == 1:
                    ...
                    print(f"   ⏭️ Skipping column '{column_name}'")
                    return None
                else:
    ...
    def _get_mandatory_simple_user_decision(self, column_name: str, suggested_field: str, 
                                          confidence: float, sample_data: pd.Series) -> Optional[Tuple[str, float, str]]:
        """Obtiene decisión OBLIGATORIA del usuario sin alternativas"""
        
        sample_str = ', '.join([str(x) for x in sample_data.head(5).tolist()])
        print(f"   📊 Sample data: {sample_str}")
        print(f"   🎯 System suggests: {suggested_field} (confidence: {confidence:.3f})")
        print(f"   ⚠️ MANUAL CONFIRMATION REQUIRED")
        
        print(f"\n   Choose an option:")
        print(f"   1. ✅ Accept suggestion: {suggested_field}")
        print(f"   2. 🔄 Choose different field")
        print(f"   3. ⏭️ Skip this column")
    ...
    def _get_mandatory_user_decision_with_alternatives(self, column_name: str, suggested_field: str, 
        ...
        """Obtiene decisión OBLIGATORIA del usuario mostrando alternativas"""
        ...
        print(f"   🧠 Intelligent alternatives based on content analysis:")
        ...
        print(f"\n   ⚠️ MANUAL CONFIRMATION REQUIRED - Choose an option:")
        print(f"   1. ✅ Accept suggestion: {suggested_field}")
        ...
        for field_type, alt_confidence, justification in alternatives:
            print(f"   {option_counter}. 🔄 Use alternative: {field_type}")
            option_map[option_counter] = (field_type, alt_confidence, 'user_correction')
            option_counter += 1
        
        print(f"   {option_counter}. 🎯 Manual field selection")
        print(f"   {option_counter + 1}. ⏭️ Skip this column")
        
        # Obtener input del usuario
        while True:
            try:
                user_input = input(f"   👤 Your choice (1-{option_counter + 1}): ").strip()
                ...
                if choice in option_map:
                    ...
                    print(f"   ❌ Invalid choice. Please enter 1-{option_counter + 1}")
                    
            except ValueError:
                print(f"   ❌ Please enter a number (1-{option_counter + 1})")
            except KeyboardInterrupt:
                print(f"\n   ⏹️ Training interrupted by user")
                return None
    
    def _get_manual_mapping_decision(self, column_name: str, sample_data: pd.Series) -> Optional[Tuple[str, float, str]]:
        """Obtiene decisión manual para columnas sin mapeo automático"""
        ...
        print(f"\n   Choose an option:")
        print(f"   1. 🎯 Manual field selection")
        print(f"   2. ⏭️ Skip this column")
        
        while True:
            try:
                user_input = input(f"   👤 Your choice (1-2): ").strip()
    ...
    def _get_manual_field_selection(self, column_name: str, sample_data: pd.Series) -> Optional[Tuple[str, float, str]]:
        """Permite selección manual de tipo de campo usando SOLO campos estándar"""
        ...
        while True:
            try:
                user_input = input(f"   👤 Select field type number: ").strip()
                ...
                if 1 <= choice <= len(self.standard_fields):
                    ...
                    print(f"   ✅ Selected: {field_type}")
                    return (field_type, 0.8, 'manual_selection')
                else:
                    print(f"   ❌ Invalid selection. Please enter 1-{len(self.standard_fields)} or 99")
    ...
    def _show_help(self):
        ...
        print(f"\n   🆘 MANUAL CONFIRMATION TRAINER HELP")
        print(f"   " + "=" * 40)
        print(f"   This trainer requires MANUAL CONFIRMATION for ALL detections.")
        print(f"   ")
        print(f"   🔧 NEW STANDARD FIELDS ({len(self.standard_fields)}):")
        for field in self.standard_fields:
            print(f"   • {field}")
        print(f"   ")
        print(f"   🧠 INTELLIGENT ANALYSIS:")
        print(f"   • Detects data types and suggests relevant alternatives")
        print(f"   • High confidence (> 0.8): No automatic alternatives shown")
        print(f"   • Low confidence (≤ 0.8): Shows intelligent alternatives")
        print(f"   • Manual selection always available regardless of confidence")
        print(f"   • Learns new synonyms and regex patterns automatically")
        print(f"   • Updates YAML configuration files")
        print(f"   ")
        print(f"   ⚠️ MANUAL CONFIRMATION:")
        print(f"   • ALL detections require your approval")
        print(f"   • System provides suggestions but YOU decide")
        print(f"   • Your decisions help improve the system")
        print(f"   ")
        print(f"   ⌨️ COMMANDS:")
        print(f"   • 'help' or 'h': Show this help")
        print(f"   • Ctrl+C: Stop training")
        print(f"   ")
    ...
    def _record_mapping_decision(self, column_name: str, field_type: str, 
        ...
        """Registra una decisión de mapeo para aprendizaje"""
    ...
    def _learn_from_user_decision(self, column_name: str, field_type: str, sample_data: pd.Series):
        """Aprende de la decisión del usuario añadiendo sinónimos y patrones"""
        try:
            # 1. Añadir sinónimo si no existe
            self._add_new_synonym(column_name, field_type)
            
            # 2. Generar y añadir patrón regex preciso
            ...
            print(f"   ⚠️ Error learning from decision: {e}")
    
    def _add_new_synonym(self, column_name: str, field_type: str):
        """Añade nuevo sinónimo usando la función del sistema - CORREGIDO"""
        try:
            # Importar y usar DynamicFieldDefinition para añadir sinónimo
            ...
            if field_type in field_definitions:
                ...
                # Verificar si el sinónimo ya existe
                ...
                if normalized_column not in [s.lower() for s in all_synonyms]:
                    ...
                    if success:
                        # CORRECCIÓN: GUARDAR LOS CAMBIOS AL ARCHIVO YAML
                        ...
                        print(f"   ✅ Added new synonym: '{column_name}' -> {field_type}")
                    else:
                        print(f"   ⚠️ Could not add synonym '{column_name}'")
                else:
                    print(f"   ℹ️ Synonym '{column_name}' already exists for {field_type}")
            else:
                print(f"   ⚠️ Field type '{field_type}' not found in definitions")
                
        except Exception as e:
            print(f"   ⚠️ Error adding synonym: {e}")

    def _save_updated_field_definition_to_yaml(self, field_type: str, field_def):
        """NUEVA FUNCIÓN: Guarda la definición actualizada al archivo YAML"""
        try:
            # Cargar configuración actual del archivo YAML
            ...
            # Actualizar la definición del campo específico
            ...
            print(f"   💾 Updated synonyms saved to: {self.dynamic_fields_file}")
            
        except Exception as e:
            print(f"   ⚠️ Error saving field definition: {e}")
    ...
    def _generate_precise_regex_pattern(self, sample_data: pd.Series) -> Optional[str]:
        """Genera un patrón regex preciso basado en los datos de muestra"""
        try:
            if len(sample_data) == 0:
                return None
            
            # Analizar los primeros 10 valores para encontrar patrón común
            sample_values = [str(val) for val in sample_data.head(10) if pd.notna(val)]
            
            if not sample_values:
                return None
            
            # Encontrar patrón común más específico
            ...
            print(f"   ⚠️ Error generating regex pattern: {e}")
            return None
    
    def _find_common_pattern(self, values: List[str]) -> Optional[str]:
        """Encuentra un patrón común en los valores"""
        try:
            if not values:
                return None
            ...
            if len(set(lengths)) == 1:
                ...
                # Analizar tipos de caracteres por posición
                pattern_chars = []
    ...
    def _convert_to_precise_regex(self, pattern: str) -> str:
        """Convierte patrón a regex preciso"""
        try:
            regex_parts = []
            i = 0
            ...
            while i < len(pattern):
                ...
                elif char == 'U':  # Mayúsculas
                    regex_parts.append(f'[A-Z]{{{count}}}')
                elif char == 'L':  # Minúsculas
                    regex_parts.append(f'[a-z]{{{count}}}')
                elif char == 'A':  # Letras
                    regex_parts.append(f'[A-Za-z]{{{count}}}')
                elif char == 'P':  # Puntuación
    ...
    def _add_new_regex_pattern(self, field_type: str, regex_pattern: str, sample_data: pd.Series):
        ...
        try:
            ...
            if regex_pattern not in existing_patterns:
                ...
                # Añadir entrada en learning_history
                history_entry = {
                ...
                print(f"   ✅ Added precise regex pattern for {field_type}: {regex_pattern}")
            else:
                print(f"   ℹ️ Regex pattern already exists for {field_type}")
                
        except Exception as e:
            print(f"   ⚠️ Error adding regex pattern: {e}")
    ...
    def _analyze_initial_state(self) -> Dict:
        ...
        try:
            ...
            print(f"📊 Initial DataFrame Analysis:")
            print(f"   • Rows: {analysis['total_rows']}")
            print(f"   • Columns: {analysis['total_columns']}")
            print(f"   • Column names: {', '.join(analysis['column_names'][:5])}{'...' if len(analysis['column_names']) > 5 else ''}")
            
            return analysis
            
        except Exception as e:
            print(f"⚠️ Could not analyze initial state: {e}")
            return {}
    
    def _finalize_training(self) -> Dict:
        """Finaliza el entrenamiento y guarda resultados"""
        try:
            print(f"\n🎯 Finalizing MANUAL CONFIRMATION Training Session...")
    ...
    def _generate_mapping_table(self) -> List[Dict]:
        """Genera tabla con Campo Estándar | Columna Mapeada | Confianza"""
    ...
    def _create_transformed_csv(self) -> str:
        """Crea dos CSV: uno de cabecera y otro de detalle según especificaciones"""
        try:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            ...
            if 'amount' not in transformed_df.columns:
                ...
                if 'debit_amount' in transformed_df.columns and 'credit_amount' in transformed_df.columns:
                    print(f"🔧 Creating 'amount' field from debit_amount - credit_amount")
                    
                    try:
                        # FUNCIÓN MEJORADA para convertir valores a números
                        def convert_to_number(value):
                            """
                            Convierte un valor a número manejando diferentes formatos de separadores
                            """
                            ...
                            try:
                                # Casos específicos de identificación del separador decimal
                                
                                # CASO 1: No hay separadores -> número entero
                                ...
                                # CASO 2: Solo punto
                                    ...
                                    if len(dot_parts) == 2:
                                        # Si la parte después del punto tiene 1-2 dígitos -> decimal
                                        ...
                                        # Múltiples puntos: verificar si el último es decimal
                                        last_part = dot_parts[-1]  # Última parte después del último punto
                                        if len(last_part) <= 2 and len(last_part) > 0:
                                            # El último punto es decimal, los anteriores son miles
                                            # 11.384.85 -> 11384.85
                                ...
                                # CASO 4: Tanto punto como coma
                                    ...
                                    if last_dot_pos > last_comma_pos:
                                        # Punto es el último -> punto es decimal, coma es miles
                                        # Verificar que la parte decimal tenga 1-2 dígitos
                                        decimal_part = cleaned_value[last_dot_pos + 1:]
                                        if len(decimal_part) <= 2:
                                            result = float(cleaned_value.replace(',', ''))
                                        else:
                                            # Demasiados dígitos después del punto -> no es decimal
                                            result = float(cleaned_value.replace('.', '').replace(',', ''))
                                    else:
                                        # Coma es el último -> coma es decimal, punto es miles
                                        # Verificar que la parte decimal tenga 1-2 dígitos
                                        decimal_part = cleaned_value[last_comma_pos + 1:]
                                        if len(decimal_part) <= 2:
                                            result = float(cleaned_value.replace('.', '').replace(',', '.'))
                                        else:
                                            # Demasiados dígitos después de la coma -> no es decimal
                                            result = float(cleaned_value.replace('.', '').replace(',', ''))
                        ...
                        # Aplicar conversión línea a línea a las columnas
                        ...
                        # Calcular amount = debit_amount - credit_amount línea a línea
                        transformed_df['amount'] = debit_values - credit_values
                        
                        print(f"✓ Created 'amount' field successfully")
                        ...
                        print(f"⚠️ Error creating amount field: {e}")
                        # Crear campo con valores 0 como fallback
                        transformed_df['amount'] = 0.0
                else:
                    print(f"⚠️ Cannot create 'amount' field: debit_amount and/or credit_amount not available")
                    print(f"   Available columns: {list(transformed_df.columns)}")
            else:
                print(f"✓ Field 'amount' already exists in transformed data")
            ...
            try:
                ...
                def separate_datetime_field(df, field_name):
                    ...
                    for value in sample_values:
                        ...
                        if any(re.search(pattern, str_value) for pattern in combined_datetime_patterns):
            ...
            if available_header_cols:
                ...
                if 'journal_entry_id' in available_header_cols:
                    ...
                    print(f"⚠️ No journal_entry_id found for grouping. Using first row as header sample.")
                ...
                print(f"⚠️ No header fields found in mapped columns")
                header_file = None
            ...
            if available_detail_cols:
                ...
                print(f"⚠️ No detail fields found in mapped columns")
                detail_file = None
            ...
            print(f"⚠️ Could not create transformed CSV files: {e}")
            return {"header_file": None, "detail_file": None, "error": str(e)}

    def _handle_datetime_fields(self, df):
        """Maneja la separación de campos DateTime en entry_date y entry_time - VERSIÓN CORREGIDA"""
        try:
            print("🔧 Checking for combined DateTime fields (improved detection)...")
                ...
                if col in ['entry_date', 'entry_time'] and col in df.columns:
                    ...
                    for value in sample_values:
                        if pd.notna(value):
                            ...
                            # PRIMERO: Verificar si es fecha pura
                            pure_date_patterns = [
                                r'^\d{1,2}\.\d{1,2}\.\d{4}$',      # DD.MM.YYYY
                                r'^\d{1,2}/\d{1,2}/\d{4}$',       # DD/MM/YYYY
                                r'^\d{1,2}-\d{1,2}-\d{4}$',       # DD-MM-YYYY
                            ...
                            # TERCERO: Solo si NO es fecha pura NI tiempo puro, verificar datetime combinado
                            # Verificar si contiene tanto fecha como hora (espacios, etc.)
                            datetime_patterns = [
                                r'\d{4}-\d{2}-\d{2}\s+\d{1,2}:\d{2}',        # YYYY-MM-DD HH:MM
                                r'\d{1,2}/\d{1,2}/\d{4}\s+\d{1,2}:\d{2}',    # DD/MM/YYYY HH:MM
                                r'\d{1,2}-\d{1,2}-\d{4}\s+\d{1,2}:\d{2}',    # DD-MM-YYYY HH:MM
                    ...
                    # Solo agregar a candidatos si realmente tiene datetime combinado
                    # NO si es solo fechas puras o solo tiempos puros
                    total_samples = len(sample_values)
                    if total_samples > 0:
                        pure_date_ratio = pure_date_count / total_samples
                        pure_time_ratio = pure_time_count / total_samples
                        
                        # Si la mayoría son fechas puras o tiempos puros, NO procesar
                        if pure_date_ratio >= 0.7:
                            print(f"   ℹ️ Field '{col}' contains pure dates, NOT processing")
                            continue
                        elif pure_time_ratio >= 0.7:
                            print(f"   ℹ️ Field '{col}' contains pure times, NOT processing")
                            continue
                        elif datetime_detected:
                            datetime_candidates.append(col)
                            print(f"   📅 Field '{col}' contains combined date+time, will process")
            
            # Procesar solo columnas que realmente tienen fecha y hora combinadas
            for col in datetime_candidates:
                print(f"🔍 Processing combined DateTime field: {col}")
                
                try:
                    # IMPORTANTE: Solo procesar si realmente contiene datetime combinado
                    # NO usar pd.to_datetime(errors='coerce') que inventa fechas
                    ...
                    for value in df[col]:
                        ...
                        # Verificar si realmente contiene fecha y hora
                        has_space_and_colon = ' ' in str_value and ':' in str_value
                        
                        if has_space_and_colon:
                            try:
                                # Solo ahora intentar parsear
                                parsed_dt = pd.to_datetime(str_value, errors='raise')
                                
                                # Extraer fecha manteniendo formato original si es posible
                                if '.' in str_value:
                                    date_str = parsed_dt.strftime('%d.%m.%Y')
                                elif '/' in str_value:
                                    date_str = parsed_dt.strftime('%d/%m/%Y')
                    ...
                    if any(time for time in times if time):
                        if col == 'entry_date' and 'entry_time' not in df.columns:
                            df['entry_date'] = dates
                            df['entry_time'] = times
                            print(f"✓ Split {col} into entry_date and entry_time")
                        elif col == 'entry_time' and 'entry_date' not in df.columns:
                            df['entry_date'] = dates
                            df['entry_time'] = times
                            print(f"✓ Split {col} into entry_date and entry_time")
                        else:
                            print(f"   ℹ️ Both entry_date and entry_time already exist, skipping {col}")
                    else:
                        print(f"   ℹ️ No valid datetime combinations found in {col}")
                            
                except Exception as e:
                    print(f"⚠️ Could not process DateTime field {col}: {e}")
                        
        except Exception as e:
            print(f"⚠️ Error in datetime handling: {e}")
        
        print("✓ DateTime field handling completed (improved logic)")


def run_batch_analysis_manual_confirmation(csv_file: str, erp_hint: str = None) -> Dict:
    """
    Análisis en lote con confirmación manual requerida
    NOTA: En modo batch, se muestran las recomendaciones pero no se requiere interacción
    """
    print(f"🔍 MANUAL CONFIRMATION BATCH ANALYSIS")
    print(f"=" * 40)
    print(f"📂 File: {csv_file}")
    print(f"🧠 Enhanced content analysis: ACTIVE")
    print(f"ℹ️ Showing recommendations that would require manual confirmation")
    ...
    try:
        ...
        for column_name in df.columns:
            ...
            if mapping_result:
                ...
                # Verificar si está en campos estándar
                if field_type not in standard_fields:
                    print(f"⚠️ {column_name}: {field_type} -> NOT IN STANDARD FIELDS")
                    continue
                
                mappings[column_name] = field_type
                print(f"🤖 {column_name}: {field_type} ({confidence:.3f}) - WOULD REQUIRE CONFIRMATION")
                ...
                sample_str = ', '.join([str(x) for x in sample_data.head(3).tolist()])
                print(f"❓ {column_name}: No mapping found")
        ...
        print(f"\n📊 MANUAL CONFIRMATION ANALYSIS RESULTS:")
        print(f"   • Detection rate: {detection_rate:.1f}%")
        print(f"   • All detections would require manual confirmation")
        print(f"   • Standard fields only: {len(standard_fields)} available")
        print(f"   • Detected columns: {len(mappings)}")
        print(f"   • No mapping: {len(df.columns) - len(mappings)}")
        ...
        print(f"❌ Error in manual confirmation batch analysis: {e}")
        return {'success': False, 'error': str(e)}


def quick_train_manual_confirmation(csv_file: str, erp_hint: str = None) -> Dict:
    """
    Función principal para entrenamiento con confirmación manual obligatoria
    """
    print(f"🚀 MANUAL CONFIRMATION Interactive Training Started")
    print(f"=" * 55)
    print(f"⚠️ ALL detections will require manual confirmation")
    ...
    try:
        ...
        # Crear sesión de entrenamiento con confirmación manual
        ...
        # Ejecutar entrenamiento interactivo con confirmación manual obligatoria
        result = session.run_interactive_training()
        
        if result['success']:
            print(f"\n🎉 MANUAL CONFIRMATION TRAINING COMPLETED SUCCESSFULLY!")
            print(f"📊 Statistics:")
            ...
            print(f"   • New synonyms added: {result['training_stats']['synonyms_added']}")
            print(f"   • New regex patterns: {result['training_stats']['regex_patterns_added']}")
            print(f"   • All confirmations manual: {result['training_stats']['all_confirmed_manually']}")
            print(f"   • Standard fields only: {len(session.standard_fields)}")
        
        return result
        
    except Exception as e:
        print(f"❌ Manual confirmation training failed: {e}")
        return {'success': False, 'error': str(e)}
...
def main():
    """Función principal"""
    if len(sys.argv) < 2:
        print("📋 MANUAL CONFIRMATION TRAINER")
        print("=" * 50)
        print("Training with MANUAL CONFIRMATION for ALL detections")
        print()
        print("🔧 NEW FEATURES:")
        print("  • ALL detections require manual confirmation")
        print("  • High confidence (> 0.8): No automatic alternatives")
        print("  • Low confidence (≤ 0.8): Shows intelligent alternatives")
        print("  • Manual field selection always available")
        print("  • New standard fields (17 fields total)")
        print("  • Automatic synonym learning")
        print("  • Precise regex pattern generation")
        print("  • YAML configuration updates")
        print("  • Final mapping table report")
        print()
    ...
    if sys.argv[1] == "--batch":
        if len(sys.argv) < 3:
            print("❌ Batch mode requires CSV file")
            return
